...
The browser and NDEF interfaces for chips allows for reading and writing commands to the chips according to the [command structure](../HaLo/command.mdx). The browser interface uses the credential API which is available in most mobile browsers on NFC-enabled smartphones including Mobile Safari and Android Chrome.
