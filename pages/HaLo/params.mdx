...
<Callout type="info">
Significant parameter changes occurred with version `c5`.
</Callout>

## Redirecting a HaLo to a new URL

Currently all HaLos are fixed to the `https://eth.vrfy.ch/` URL. This URL follows the redirects from the onchain Ethereum Reality Service ("ERS") registry. See [ERS](../ERS/overview) for more information on how to register a HaLo chip with the ERS registry to redirect it to a desired URL. All parameters in the URL are included on redirect.

<Tabs items={['Versions c1-c4', 'Version c5+']} defaultIndex="1">
  <Tab>
  ## Example URL
  ...
  **Example `res` field value:**
  ...
  In the example shown here, the `pk3` has not been initialized, `latch1` has not been set, and `latch2` has been set. 
